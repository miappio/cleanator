...
.controller('ChoresCtrl', function ($scope, $timeout,$stateParams,
...
.controller('ChoresDetailCtrl', function ($scope, $timeout,$stateParams,srvData, srvConfig) {
  ...
  $scope.choreSaveDetail = function() {
    ...
    .then(function (res) {
      ...
      // is it an add ? -> refresh chore list $scope.chores ?
      //if (reBindData === true) $scope.choreDataSync();
      //navScrollToSet(choreId);
      //$scope.$emit("chore.changed");
      //$scope.navDataSync().finally(function(){
      //  $scope.navRedirect("config/categorychores/"+$scope.choreToEdit[$scope.choreCols.category]);
      //});
		})
		.catch(function(err){
			//$scope.choreSaveJeton = false;
