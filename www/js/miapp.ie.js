...
// ze baas  u box
/**
 *  Miapp.setEndPoint("http://localhost:9000"); //the address of your BaasBox server
 * Miapp.appcode = "1234567890";               //the application code of your server
 *
 *
 * Miapp.signup("new", "password")
 .done(function (res) {
  ...
  /*
    ...
    $.ajaxSetup({
       ...
       beforeSend: function(r, settings) {
         if (Miapp.getCurrentUser()) {
           r.setRequestHeader('X-MIAPP-SESSION', Miapp.getCurrentUser().token);
         }
         r.setRequestHeader('X-MIAPP-APPCODE', Miapp.appcode);
       }
    });
*/
    function createInstance() {
      var object = new Object("I am the Miapp instance");
      return object;
    }
    ...
    function buildDeferred() {
      //var dfd = new $q.Deferred();
      var deferred = angular.$q.defer();
      var promise = {};
    ...
    return {
      ...
      // role constants, by default in the Miapp back end
      ...
      login: function(user, pass) {
        ...
        var url = Miapp.endPoint + '/login';
        var loginRequest = $.post(url, {
          username: user,
          password: pass,
          appcode: Miapp.appcode
        })
        .done(function(res) {
          var roles = [];
          ...
          setCurrentUser({
            ...
            "token": res.data['X-MIAPP-SESSION'],
            "roles": roles,
      ...
      signup: function(user, pass, acl) {
        ...
        var req = angular.$http({
          ...
          .done(function (res) {
            var roles = [];
            //$(res.data.user.roles).each(function(idx, r) {
            res.data.user.roles.forEach(function(idx, r) {
              roles.push(r.name);
            });
            setCurrentUser({
              "username": res.data.user.name,
      ...
      fetchCurrentUser: function () {
        return $.get(Miapp.endPoint + '/me')
      },

      createCollection: function(collection) {
        return $.post(Miapp.endPoint + '/admin/collection/' + collection)
      },

      deleteCollection: function(collection) {
        return $.ajax({
          url: Miapp.endPoint + '/admin/collection/' + collection,
          method: 'DELETE'
        })
      },
      ...
      loadCollectionWithParams: function(collection, params) {
        var deferred = buildDeferred();
        var url = Miapp.endPoint + '/document/' + collection;
        var req = $.ajax({
            url: url,
            method: 'GET',
      ...
      loadCollection: function(collection) {
        return Miapp.loadCollectionWithParams(collection, {page: 0, recordsPerPage: Miapp.pagelength});
      },

      loadObject: function (collection, objectId) {
        return $.get(Miapp.endPoint + '/document/' + collection + '/' + objectId)
      },
      ...
      save: function(object, collection) {
        ...
        if (object.id) {
          method = 'PUT';
          url = Miapp.endPoint + '/document/' + collection + '/' + object.id;
        }
      ...
      updateField: function(objectId, collection, fieldName, newValue) {
        var deferred = buildDeferred();
        url = Miapp.endPoint + '/document/' + collection + '/' + objectId + '/.' + fieldName;
      ...
      updateObject: function(objectId, collection, newData) {
        var deferred = buildDeferred();
        url = Miapp.endPoint + '/document/' + collection + '/' + objectId;
        var req = $.ajax({
          url: url,
          type: 'PUT',
          contentType: 'application/json',
      ...
      fetchObjectsCount: function (collection) {
        return $.get(Miapp.endPoint + '/document/' + collection + '/count');
      },

      grantUserAccessToObject: function (collection, objectId, permission, username) {
        return $.ajax({
            url: Miapp.endPoint + '/document/' + collection + '/' + objectId + '/' + permission + '/user/' + username,
            method: 'PUT'
        });
      },
      ...
      grantRoleAccessToObject: function (collection, objectId, permission, role) {
        return $.ajax({
            url: Miapp.endPoint + '/document/' + collection + '/' + objectId + '/' + permission + '/role/' + role,
            method: 'PUT'
        });
      },
      ...
      loadAssetData: function(assetName) {
        var deferred = buildDeferred();
        var url = Miapp.endPoint + '/asset/' + assetName + '/data';
        var req = $.ajax({
          url: url,
          method: 'GET',
          contentType: 'application/json',
      ...
      getImageURI: function(name, params) {
        var deferred = buildDeferred();
        var uri = Miapp.endPoint + '/asset/' + name;
        var r;
        if (params === null || this.isEmpty(params)) {
          return deferred.resolve({"data": uri + "?X-MIAPP-APPCODE=" + Miapp.appcode})
        }
        ...
        uri = uri.concat(r);
        p = {};
        p['X-MIAPP-APPCODE'] = Miapp.appcode;
        var req = $.get(uri, p)
          .done(function(res) {
      ...
      fetchUserProfile: function (username) {
        return $.get(Miapp.endPoint + '/user/' + username)
      },

      fetchUsers: function (params) {
        return $.ajax({
            url: Miapp.endPoint + '/users',
            method: 'GET',
            data: params
        });
      },
      ...
      changePassword: function (oldPassword, newPassword) {
        return $.ajax({
            url: Miapp.endPoint + '/me/password',
            method: 'PUT',
            contentType: 'application/json',
      ...
      resetPassword: function() {
        ...
        return $.get(Miapp.endPoint + '/user/' + user.username + '/password/reset');
      },

      followUser: function (username) {
        return $.post(Miapp.endPoint + '/follow/' + username);
      },

      unfollowUser: function (username) {
        return $.ajax({
          url: Miapp.endPoint + '/follow/' + username,
          method : 'DELETE'
        });
      },

      fetchFollowers: function (username) {
        return $.get(Miapp.endPoint + '/followers/' + username);
      },

      fetchFollowing: function (username) {
        return $.get(Miapp.endPoint + '/following/' + username);
      },

	    sendPushNotification: function(params) {
        return $.ajax({
          url: Miapp.endPoint + '/push/message',
          method: 'POST',
          contentType: 'application/json',
          data: JSON.stringify(params)
        })
      ...
      fetchFile: function(fileId) {
        return $.get(Miapp.endPoint + '/file/' + fileId + "?X-MIAPP-SESSION=" + Miapp.getCurrentUser().token)
      },

      deleteFile: function(fileId) {
        return $.ajax({
          url: Miapp.endPoint + '/file/' + fileId,
          method : 'DELETE'
        });
      },

      fetchFileDetails: function(fileId) {
        return $.get(Miapp.endPoint + '/file/details/' + fileId)
      },

      grantUserAccessToFile: function (fileId, permission, username) {
        return $.ajax({
            url: Miapp.endPoint + '/file/' + fileId + '/' + permission + '/user/' + username,
            method: 'PUT'
        });
      },
      ...
      grantRoleAccessToFile: function (fileId, permission, rolename) {
        return $.ajax({
            url: Miapp.endPoint + '/file/' + fileId + '/' + permission + '/role/' + rolename,
            method: 'PUT'
        });
      },
      ...
      callPlugin: function(pluginName, method, data) {
        var deferred = buildDeferred();
        var req = $.ajax({
          url: Miapp.endPoint + '/plugin/'+pluginName,
