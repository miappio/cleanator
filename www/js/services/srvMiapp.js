angular.module('srvMiapp', [])
    .factory('srvMiapp', function ($log,$q,$timeout) {

        return new SrvMiapp($log,$q,$timeout);
    });
...
'use strict';
    ...
    Service.prototype.login = function (login, password, updateProperties) {
        ...
        // console.log('miappClient.loginMLE : '+ login+ ' / '+ encrypted_json_str);

        self.miappClient.loginMLE(login,encrypted_json_str,updateProperties,function (err, user) {
            // console.log('callback done :' + err + ' user:' + user);
            if (err) {
    ...
    Service.prototype.deleteUser = function (userIDToDelete) {
        ...
        self.miappClient.deleteUserMLE(userIDToDelete,function (err) {
            // console.log('deleteUserMLE callback done :' + err);
            if (err) {
                // Error - could not log user in
                return defer.reject(err);
            } else {
    ...
    */
    ...
    Service.prototype.isPouchDBEmpty = function (pouchDB) {
        ...
        pouchDB.allDocs({
                ...
                console.log('isPouchDBEmpty callback');
                if (err) return deferred.reject(err);

                if (response && response.total_rows && response.total_rows > 5) return deferred.resolve(false);

                console.log('isPouchDBEmpty callback: '+ response.total_rows);
